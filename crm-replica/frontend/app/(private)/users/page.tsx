...
<th className="p-2" />
          ...
          <tbody>
            {users.map((u) => <tr key={u.id} className="border-t border-[var(--border)]">
<td className="p-2">{u.first_name} {u.last_name}</td>
<td className="p-2">{u.email}</td>
<td className="p-2">
<div className="flex items-center gap-2">
<Badge className={u.role === 'admin' ? 'border-blue-500 text-blue-300' : 'border-amber-500 text-amber-300'}>{u.role}</Badge>{u.id !== me?.id ? <Select value={u.role} disabled={updatingUserId === u.id} onChange={(event) => void updateRole(u, event.target.value as 'admin' | 'tecnico')} className="max-w-36">
<option value="admin">admin</option>
<option value="tecnico">tecnico</option>
</Select> : <span className="text-xs text-[var(--text-secondary)]">Usuario actual</span>}</div>
</td>
